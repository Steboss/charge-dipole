...
def BarkerWatts(na_coords,res_coords,rcutoff,distionCOM):
...
def compute_ion_dipole_rotationally_avg(na_coords,COM,rcutoff):
	...
	charge_term = inv_KT*((qion*mu)**2)*(one_over_four_pi_eps0**2)*(1/r)**4
	first_rf = (epsilon2 + 1)**2
	sec_rf = (4*ratio)*(epsilon1)*((ratio*epsilon1)-1-epsilon2)
	pot = charge_term*(first_rf + sec_rf)
...
def readdcd(frame,framenumb,top,rcutoff,width=1.0,pdb=False,pdbcomoxy=False):
	...
	for res in top.residues:
		if "HOH" in res.name: #if the residue is water
			...
			#BWRF energy
			nrg = BarkerWatts(na_coords,res_coords,rcutoff,dist)
			...
			if shell in shell_coords: 
				...
				RF_charge_dipole[shell]+=ion_dipole_nrg # RF ion dipole interaction
				AVG_RF_charge_dipole[shell]+=ion_dipole_rot_nrg#Rot AVG RF ion dipole interaction, with
	...
	return BWRF,RF_charge_dipole,AVG_RF_charge_dipole,shell_coords,angles
...
for framenumber in range(start,end):
	...
	#main function:
	nrg_BWRF,nrg_RF_charge_dipole,nrg_AVG_RF_charge_dipole,shells,angles_shell= readdcd(current,framenumber,mdtrajtop,rcutoff,width)
	...
	for key in shells.keys():
		if key in BWRF: #this means that the key is in all the dictionaries
			BWRF[key].append(nrg_BWRF[key])
			RF_charge_dipole[key].append(nrg_RF_charge_dipole[key])
			AVG_RF_charge_dipole[key].append(nrg_AVG_RF_charge_dipole[key])
			#compute the average angle for each shell
			angles_dict[key].append(np.mean(angles_shell[key]))

		else:
			BWRF[key]=[nrg_BWRF[key]]
			RF_charge_dipole[key]=[nrg_RF_charge_dipole[key]]
			AVG_RF_charge_dipole[key]=[nrg_AVG_RF_charge_dipole[key]]
...
for key in RF_charge_dipole.keys():
	...
	RF_charge_dipole_file.write("%.4f,%.8f,%.8f\n" % (key,RF_charge_dipole[key][0],RF_charge_dipole[key][1]))
...
for key in AVG_RF_charge_dipole.keys():
	...
	AVG_RF_file.write("%.4f,%.8f,%.8f\n" % (key,AVG_RF_charge_dipole[key][0],AVG_RF_charge_dipole[key][1]))
...
for key in AVG_RF_charge_dipole_integrated.keys():
	...
	AVG_RF_int_file.write("%.4f,%.8f,%.8f\n" % (key,AVG_RF_charge_dipole_integrated[key][0],AVG_RF_charge_dipole_integrated[key][1]))

#add a readme to each folder
readme_file = open("%s/README" % outputfold,"w")
readme_file.write("""BWRF.csv:  Coulombic interaction under Barker Watts reaction field (charge-charge-charge)
RF_charge_dipole.csv : Charge-Dipole reaction field interaction (dipole-ion)
AVG_RF_dipole.csv: Rotationally Averaged charge-dipole reaction field interaction, setting position r with COM positions
AVG_RF_int_dipole.csv: Rotationally Averaged charge-dipole reaction field interaction, computed as N_waters*potential, setting molecules in the middle of each shell 
