...
#algorithm:
#Compute the interaction ion-water, rotation the water around the z-axis
#for each theta of rotation compute the interaction
#compute the interaction as a normal Pythagora's calculation for the moment
#The oxygen will remain in the origin
#At each step we must know where is te dipole, so let's suppose that the dipole splits
#the water angle in twice
...
#rotate 180 for convection
    ...
    outputfold = "%d_deg" % (math.ceil(theta_deg))
    ...
    print("Angle  of rotation %.2f" % theta_deg)
    ...
    for r in r_vector:
       ...
       #now compute ion-oxygen, ion-hydrogen1/2
       rO = pythag(ioncoord,(0,0,0))
       rH1 = pythag(ioncoord,h1coords)
       rH2 = pythag(ioncoord,h2coords)
       #Barker Watts Coulombic energy
       nrg = BarkerWatts(qion,o_charge,h1charge,h2charge,rO,rH1,rH2,cutoff_dist.val.value(),rf_dielectric.val)
       current_angle = theta +  pi # the pi additional is due to the  convection for the dipole, from - to plus, here we 
       #are going from + to -
       #should we consider r(ion-oxygen) as the distance from the ion to the dipole?
       rf_nrg = derived_RF(qion,rO,current_angle,cutoff_dist.val.value(),rf_dielectric.val)
