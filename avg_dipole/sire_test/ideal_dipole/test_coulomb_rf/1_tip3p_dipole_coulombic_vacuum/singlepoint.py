...
def dipole_rotate(system,theta):
    ...
    new_coords = AtomCoords(dipole.property("coordinates"))
    #now set the new coords for h1 and h2
    new_coords.set(cgh1,newh1)
    new_coords.set(cgh2,newh2)
    #commit the new coordinates
    dipole = dipole.edit().setProperty("coordinates",new_coords).commit()
    #add the atoms to the changedmols group and update the system
    changedmols.add(dipole)
...
def Coulomb(qion,h1,h2,r1,r2):
    ...
    pot_ion1 = one_over_four_pi_eps0*(qion*h1)*( (1/r1))
    pot_ion2 = one_over_four_pi_eps0*(qion*h2)*( (1/r2))
    pot = pot_ion1 + pot_ion2

    return pot

def charge_dipole_theory(distances,theta,outputfile):
    r"""Function to compute an ideal Coulombic charge-dipole interaction
    in vacuum
    ...
    nrgs = []
    for r in distances:
        qdip =  (one_over_four_pi_eps0)*(-q1*mu)*(r**-2)*math.cos(theta)
        nrgs.append(qdip)
...
bond_scale = 21# max factor we are scaling bonds and charges (20)
#Wjat we need to do:
...
#cycle 
print("Computing charge-dipole interactions theoretical predictions...")
for theta in thetas:
    theta_deg = theta*(180.0/pi)
    #output folder
    theory_output = "results/theory" 
    if not os.path.exists(theory_output):
        os.makedirs(theory_output)
    outputfile = "results/theory/%d.csv" % theta_deg
    #print("Saving results in ...")
    #print(outputfile)
    ofile = open(outputfile,"w")

    #save everyting in a dict - it is useful?
    theory[theta] = []
    nrgs = charge_dipole_theory(r_vector,theta,ofile)
    theory[theta].append(nrgs)
    ofile.close()
...
for bond in range(1,bond_scale):
    #first scale the charges and bond
    print("Bond-Charge scale factor: %.4f" % bond)
    #scale everything
    dipole_scaling(system,bond,h1h2_bond,h1_orig,h2_orig)
    #uncomment to check
    #dipole_check(system)

    #create output for scaling number bond
    scale_output = "results/scale_%d/" % bond
    if not os.path.exists(scale_output):
        os.makedirs(scale_output)

    #rotate the dipole
    for i in range(n_rotations):
        theta_deg = i*rot_angle*(180.0/pi)
        print("Theta %.4f" % theta_deg)
        #save everyting
        outputfile = scale_output + "%d.csv" % theta_deg
        ofile = open(outputfile,"w")
        #print("Saving results in ...")
        #print(outputfile)
        #rotate the dipole by 30 deg
        if i==0:
            h1_coords,h2_coords = dipole_rotate(system,0.0)
        else:
            h1_coords,h2_coords= dipole_rotate(system,rot_angle)
        #print(h1_coords,h2_coords)
        #time.sleep(2)
        #compute the theoretical behavior the first time
        computed[theta_deg,bond] = []


        for r in r_vector:
            #now for each distance compute the Coulombic term
            #first fix the ion at distance r
            ion_coords,ion_charge = sodium_settings(system,r)
            #compute the new charges of the system (scaled)
            h1_charge, h2_charge = dipole_charges(system)
            #now compute the energy of the system
            r1 = pythag(ion_coords,h1_coords)
            r2 = pythag(ion_coords,h2_coords)

            #compute the new bond lenth
            bond_length = dipole_bond(system)
            ener = Coulomb(ion_charge,h1_charge,h2_charge,r1,r2)

            #r,scaling,bond,|charges|,theta,nrg
            #outputf.write("%.4f,%d,%.4f,%.4f,%.2f,%.4f\n" % (r,bond,bond_length,h2_charge,theta_deg,nrg))
            computed[theta_deg,bond].append(ener)
            #print(theta,ener)
            ofile.write("%.4f,%.4f\n" % (r,ener))
